...
class ShfeL1MDProducerHelper
{
	public:
		/*
		 * 获取指定合约的最新行情。
		 * 从行情缓存的最新位置向前查找最多查找主力合约个数Deep位置，
		 * 中途找到则立即返回
		 */
...
class ShfeL1MDProducer : public CThostFtdcMdSpi
{
	public:
		ShfeL1MDProducer(struct vrt_queue  *queue);
		~ShfeL1MDProducer();
	...
	private:
		...
		bool ended_;
		void Convert(CDepthMarketDataField &quote_level1,
...
class ShfeL1MDProducer : public EESQuoteEvent
{
	public:
		ShfeL1MDProducer(struct vrt_queue  *queue);
		~ShfeL1MDProducer();
