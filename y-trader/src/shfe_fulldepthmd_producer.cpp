...
ShfeFullDepthMDProducer::ShfeFullDepthMDProducer(struct vrt_queue  *queue)
		:module_name_("FullDepthProducer")
{
	udp_fd_ = 0;
	ended_ = false;
	clog_warning("[%s] FULL_DEPTH_MD_BUFFER_SIZE: %d;", 
				module_name_, 
				FULL_DEPTH_MD_BUFFER_SIZE);
	...
	dominant_contract_count_ = 
		LoadDominantContracts(config_.contracts_file, dominant_contracts_);
	...
	thread_rev_ = new std::thread(&ShfeFullDepthMDProducer::RevData, this);
}

void ShfeFullDepthMDProducer::ParseConfig()
{
...
void ShfeFullDepthMDProducer::Process(MDPack *md) 
{
...
void ShfeFullDepthMDProducer::End()
{
	if(!ended_)
	{
		ended_ = true;
...
int32_t ShfeFullDepthMDProducer::Push(const MDPackEx& md){
...
MDPackEx* ShfeFullDepthMDProducer::GetData(int32_t index)
{
	return &shfemarketdata_buffer_[index];
}

bool ShfeFullDepthMDProducer::IsDominant(const char *contract)
{
...
std::string ShfeFullDepthMDProducer::ToString(const MDPackEx &d) 
{
...
int ShfeFullDepthMDProducer::Init()
{
    // init udp socket
...
void ShfeFullDepthMDProducer::RevData()
{
	int udp_fd = Init();
	udp_fd_ = udp_fd; 
    if (udp_fd < 0) 
    ...
    while (!ended_)
	{
        ...
        if (recv_len == -1) 
		{
			...
			{
                clog_error("[%s] UDP-recvfrom failed, error_no=%d.",
							module_name_,
							error_no);
                continue;
            }
