...
public:
	...
	double avg_px;				//	平均价
	double last_px;				//	最新价
	double bp_array[5];			//	多档买价
	double ap_array[5];			//	多档卖价
	int bv_array[5];			//	多档买量
	int av_array[5];			//	多档卖量
	int total_vol;				//	总成交量
	double total_notional;		//	总成交额
	double upper_limit_px;		//	涨停价
	double lower_limit_px;		//	跌停价
	double close_px;				//	收盘价 只在日盘收盘后的几笔行情有效，其余时间为0
	double settle_px;			//	结算价 只在日盘收盘后的几笔行情有效，其余时间为0
	...
	double weighted_buy_px;		//	平均买价 DCE OrderStat quote
	double weighted_sell_px;		//	平均卖价 DCE OrderStat quote	
};
